...
class VoteRepository(BaseRepository[Vote]):
    ...
    async def findByVoterNic(self, voter_nic: str) -> Optional[Vote]:
        try:
            logger.info("VoteRepository: Finding vote by voter_nic...")
            
            vote = await self.findOne(filters={"voter_nic": voter_nic})
            
            logger.info("VoteRepository: Found vote by voter_nic.")
            
            return vote
        
        except Exception as e:
            logger.error("VoteRepository: Failed to find vote by voter_nic.", exc_info=True)
            raise e
